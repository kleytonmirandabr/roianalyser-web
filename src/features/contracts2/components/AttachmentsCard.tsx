/**
 * Bloco de anexos do contrato (Sprint C).
 *
 * Drag-drop ou file picker, categoria por anexo, preview inline (PDF iframe,
 * imagem inline). Lista com remove (soft delete). Limites: 10MB/arquivo, mime
 * allowlist (pdf/png/jpg/docx).
 */
...
import {
  ...
  type AttachmentKind,
} from '@/features/contracts2/attachments-types'
import { contractAttachmentsApi } from '@/features/contracts2/attachments-api'
...
import {
  ...
  useDeleteAttachment,
  useUploadAttachment,
} from '@/features/contracts2/hooks/use-contract-attachments'
...
export function AttachmentsCard({ contractId }: { contractId: string | undefined }) {
  const list = useContractAttachments(contractId)
  ...
  // Quando usuário escolhe um arquivo: valida e abre o painel de confirmação.
  function pickFile(file: File | null | undefined) {
    ...
    setPendingFile(file)
    // Heurística de categoria: se nome contém "aditivo" pre-seleciona; default contrato.
    const lower = file.name.toLowerCase()
    if (lower.includes('aditivo')) setPendingKind('amendment')
    else if (lower.includes('comprov') || lower.includes('nota')) setPendingKind('receipt')
    else setPendingKind('contract')
    setPendingNotes('')
  }

  async function doUpload() {
    if (!pendingFile || !contractId) return
    try {
  ...
  // Preview: baixa o arquivo via blob (com auth do api client) e gera object URL
  async function openPreview(it: { id: string; filename: string; mime: string }) {
    if (!contractId) return
    setPreviewing(it)
    setPreviewBlobUrl(null)
    try {
      // Usa fetch direto pra preservar o Authorization header
      const url = contractAttachmentsApi.downloadUrl(contractId, it.id)
  ...
  const kindOptions = (Object.entries(ATTACHMENT_KIND_LABELS) as Array<[AttachmentKind, string]>).map(
  ...
  return (
    <Card className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Contrato assinado, aditivos, comprovantes. PDF, PNG, JPG ou DOCX (máx. 10MB).
          </p>
        </div>
      </div>
      ...
      {/* Painel de confirmação após escolha */}
      {pendingFile && (
        <div className="rounded-md border border-primary/30 bg-primary/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <Combobox value={pendingKind} onChange={(v) => setPendingKind(v as AttachmentKind)} options={kindOptions} />
            </div>
            <div>
              ...
              <Input
                ...
                placeholder="Ex.: Aditivo de prazo assinado em..."
                maxLength={500}
              />
            </div>
          </div>
                ...
                <div className="p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5 flex-wrap">
                      <span className="font-medium uppercase tracking-wide">{ATTACHMENT_KIND_LABELS[it.kind]}</span>
                      <span>·</span>
