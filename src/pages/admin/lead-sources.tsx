/** Admin → Fonte de Lead (master only). */
...
import {
  useCreateLeadSource, useDeleteLeadSource,
  useLeadSources, useUpdateLeadSource,
} from '@/features/lead-sources/hooks/use-lead-sources'
...
export function AdminLeadSourcesPage() {
  ...
  const update = useUpdateLeadSource(draft.id)
  const del = useDeleteLeadSource()

  if (user && !user.isMaster) return <Navigate to="/admin" replace />
  const items = (data ?? []) as LeadSource[]
  const columns = useMemo<DataTableColumn<LeadSource>[]>(() => [
  ...
  function openEdit(item: LeadSource) {
  ...
  async function handleDelete(item: LeadSource) {
    const it = item as unknown as Record<string, unknown>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{t('nav.adminLeadSources')}</h1>
          <p className="text-sm text-muted-foreground">Catálogo configurável.</p>
        </div>
        <div className="flex items-center gap-2">
          <CsvExportButton
            filename="fontes-de-lead"
            rows={(dt.rows as any[])}
            columns={[
