/** Admin → Categoria de Item (master only). */
...
import {
  useCreateItemCategory, useDeleteItemCategory,
  useItemCategories, useUpdateItemCategory,
} from '@/features/item-categories/hooks/use-item-categories'
import type { ItemCategory } from '@/features/item-categories/types'
...
export function AdminItemCategoriesPage() {
  ...
  const update = useUpdateItemCategory(draft.id)
  const del = useDeleteItemCategory()

  if (user && !user.isMaster) return <Navigate to="/admin" replace />
  const items = (data ?? []) as ItemCategory[]
  const columns = useMemo<DataTableColumn<ItemCategory>[]>(() => [
  ...
  function openEdit(item: ItemCategory) {
  ...
  async function handleDelete(item: ItemCategory) {
    const it = item as unknown as Record<string, unknown>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{t('nav.adminItemCategories')}</h1>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="sm:max-w-lg">
          <SheetHeader><SheetTitle>{draft.id ? t('admin.itemCategories.titleEdit') : t('admin.itemCategories.titleNew')}</SheetTitle></SheetHeader>
