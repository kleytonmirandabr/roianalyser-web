/** Admin → Grupo de Usuário (master only). */
...
import {
  useCreateUserGroup, useDeleteUserGroup,
  useUserGroups, useUpdateUserGroup,
} from '@/features/user-groups/hooks/use-user-groups'
import type { UserGroup } from '@/features/user-groups/types'
...
export function AdminUserGroupsPage() {
  ...
  function openEdit(item: UserGroup) {
  ...
  async function handleDelete(item: UserGroup) {
    const it = item as unknown as Record<string, unknown>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{t('nav.adminUserGroups')}</h1>
          <p className="text-sm text-muted-foreground">{t('admin.shared.configurable')}</p>
        </div>
        <div className="flex items-center gap-2">
          <CsvExportButton
            filename="grupos-de-usuario"
            rows={(dt.rows as any[])}
            columns={[
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="sm:max-w-lg">
          <SheetHeader><SheetTitle>{draft.id ? t('admin.userGroups.titleEdit') : t('admin.userGroups.titleNew')}</SheetTitle></SheetHeader>
